...
const App: React.FC = () => {
  ...
  const analyzeData = (kp: KpDataPoint[], wind: WindDataPoint[], flares: FlareDataPoint[]) => {
    ...
    let kpText = "";
    if (lastKp >= 5) {
        kpText = "СТАТУС: Активная фаза геомагнитного поля. Это естественное природное явление. Атмосфера Земли надежно защищает биосферу. Возможно наблюдение полярных сияний в средних широтах.";
    } else if (lastKp >= 3) {
        kpText = "СТАТУС: Незначительные колебания магнитосферы. Естественный природный фон, благоприятный для самочувствия.";
    } else {
        kpText = "СТАТУС: Геомагнитная обстановка спокойная и стабильная. Оптимальные условия для отдыха и работы.";
    }
    parts.push(kpText);
